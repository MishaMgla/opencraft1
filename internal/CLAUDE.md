...
- Backpressure drops frames, never blocks: `send()` evicts the oldest queued frame when a player's `out` chan (cap 64) is full; snapshots are lossy under a slow client. **Connection-critical frames must NOT use this path:** the join handshake (Welcome + the painted world + Enters) is returned by `Sim.Join` and written reliably by the connection goroutine *before* the lossy writer starts — otherwise a painted world larger than the buffer evicts the Welcome and the client never learns its id (see the common-pitfalls entry in `AGENT_RULES.md`).
