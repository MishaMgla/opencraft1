...
- `Space` requests paint for the acting player's current 128×128 floor tile; the most recent paint for a tile wins until overwritten. painted tiles persist to `public.painted_tile` (keyed on the tile origin) when `DATABASE_URL` is set, and are replayed into the painted map on startup, so the painted world survives restarts; with no `Store` (local dev/tests) they remain in-memory only and reset on process exit. restored tiles carry ownerID 0 (no live owner — runtime ids don't survive a restart).
