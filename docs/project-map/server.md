...
- `internal/server/server.go` — http mux: `/healthz` JSON liveness (`{"status":"ok"}`), `/ws` upgrade, and `/` static files (`web/`, only when that dir exists — skipped in the Railway image). WS origin policy comes from `ALLOWED_ORIGINS` (allow-all when unset). one reader loop + one writer goroutine per connection; first frame must be Hello.
