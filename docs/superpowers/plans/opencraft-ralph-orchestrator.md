# opencraft MVP — Ralph Orchestrator Prompt
...
You are the build orchestrator for the opencraft MVP. You do NOT write feature code yourself — you delegate each task to a subagent and verify its work.

The task tracker is `docs/superpowers/plans/opencraft-mvp-tasks.md`. The detailed spec (all code, interfaces, rationale) is `docs/superpowers/plans/2026-06-11-opencraft-mvp-engine.md`.

Do exactly ONE task this iteration:

1. Read the tracker. Find the FIRST task marked `[ ]` whose every dependency is `[x]`.
2. If NO task is `[ ]` → every task is done. Output `<promise>OPENCRAFT_MVP_COMPLETE</promise>` and stop.
...
5. Dispatch ONE subagent via the Agent tool (`general-purpose`) with a SELF-CONTAINED brief containing:
   ...
   - The pointer to the plan section that holds the code/spec (e.g. "implement per Plan Task 4, Step 1 in docs/superpowers/plans/2026-06-11-opencraft-mvp-engine.md — read that section and reproduce the code exactly").
...
```
/ralph-loop --completion-promise 'OPENCRAFT_MVP_COMPLETE' --max-iterations 30 You are the build orchestrator for the opencraft MVP. Follow docs/superpowers/plans/opencraft-ralph-orchestrator.md exactly. Each iteration: read the tracker docs/superpowers/plans/opencraft-mvp-tasks.md, pick the first [ ] task whose deps are all [x], mark it [~], dispatch ONE general-purpose subagent to implement it per the referenced plan section and run its verification command (subagent does NOT commit), then verify independently, commit with the task's message, and mark it [x] with the commit hash in the Progress log. One task per iteration. Never mark done without a passing verification you ran yourself. When every task is [x], output <promise>OPENCRAFT_MVP_COMPLETE</promise>.
```
