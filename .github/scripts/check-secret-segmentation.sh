...
# legitimately needs at build time. OPENROUTER_API_KEY funds image-gen credits only
# (nano-banana asset generation) — a leak burns capped credits, granting NO infra,
# data, repo, or merge access — so the Dev agent may carry it. Warned, not failed,
# like AUTO_PAT. Real production / infra secrets still hard-fail.
AGENT_PERMITTED_RE='secrets\.(OPENROUTER_API_KEY)'
...
for w in "${AGENT_WORKFLOWS[@]}"; do
  ...
  raw="$(grep -noE "$PROD_SECRET_RE" "$f" || true)"
  if [ -n "$raw" ]; then
    permitted="$(printf '%s\n' "$raw" | grep -E "$AGENT_PERMITTED_RE" || true)"
    prod="$(printf '%s\n' "$raw" | grep -vE "$AGENT_PERMITTED_RE" || true)"
    if [ -n "$permitted" ]; then
      echo "::warning file=$f::$w carries agent-permitted bounded-risk key(s) — third-party, no infra/data/repo access:"
      printf '%s\n' "$permitted" | sed 's/^/    /'
    fi
    if [ -n "$prod" ]; then
